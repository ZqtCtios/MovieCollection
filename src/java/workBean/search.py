...
def findMoive(values):
    ...
    for div in soup.find_all("div",attrs={'class':'result'}):
        try:
            title.append(div.find('div',attrs={'class':'content'}).find('div',attrs={'class':'title'}).a.text)
            page.append(div.find('div',attrs={'class': 'pic'}).a.attrs['href'])
            img.append(div.find('div',attrs={'class':'pic'}).img.attrs['src'])
            pf.append(div.find('div', attrs={'class': 'rating-info'}).find('span', attrs={'class': 'rating_nums'}).text)
