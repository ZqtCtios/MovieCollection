...
def findMoive():
    ...
    for table in soup.find_all("tr",attrs={'class':'item'}):
        try:
            ...
            pf.append(table.find_all('td', attrs={'valign':'top'})[1].find('div',attrs={'class':'star clearfix'}).find('span',attrs={'class':'rating_nums'}).text)
            msg.append(table.find_all('td', attrs={'valign':'top'})[1].find('div').p.text)
